...
class Calculator:
    ...
    def create_rns_widgets(self):
        """Создание интерфейса для калькулятора СОК"""
        main_frame = tk.Frame(self.window)
        main_frame.pack(expand=True, fill='both', padx=10, pady=10)

        # Информация о системе СОК
        info_frame = tk.LabelFrame(main_frame, text="Информация о СОК",
                                   font=('Arial', 10, 'bold'))
        info_frame.pack(fill='x', pady=(0, 10))

        info_text = """Модули: [5, 7, 11]
M = 5 × 7 × 11 = 385
Диапазон: [-192, 192]
Операции: +, -, *, //"""
        ...
        # Детали вычислений
        details_frame = tk.LabelFrame(main_frame, text="Детали вычислений",
                                      font=('Arial', 10, 'bold'))
        details_frame.pack(fill='both', expand=True, pady=(0, 10))

        self.details_text = tk.Text(details_frame, font=('Courier', 9),
                                    height=10, wrap='word')
        self.details_text.pack(fill='both', expand=True, padx=5, pady=5)

        scrollbar = tk.Scrollbar(self.details_text)
    ...
    def get_calc_type_name(self):
        ...
        names = {
            ...
            "polynomial": "Полиномы",
            "rns": "Система Остаточных Классов"
        }
    ...
    def clear(self):
        ...
        self.expression = ""
        if self.calc_type == "rns":
            # Очищаем только поле ввода, детали оставляем
            pass
        self.update_display()
    ...
    def show_result(self):
        ...
        if self.expression:
            try:
                ...
                # Для СОК показываем детали
                if self.calc_type == "rns":
                    if hasattr(self, 'rns_details') and self.rns_details:
                        self.details_text.delete(1.0, tk.END)
                        self.details_text.insert(1.0, self.rns_details)
